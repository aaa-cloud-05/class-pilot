...
| # | 論点 | 決定 |
|---|---|---|
| A | 提出済みの色 | **青**。v5 のホーム画面のとおり。ヘルプの説明図とカレンダーのドットも合わせる（→ §2A に残る詰め） |
| B | 現行の「緑＝提出済」 | **緑は捨てる** |
...
### 🔴 A. 「提出済み」の色が v5 の中で4通りに割れている（→ 青に統一する。残る詰めあり）
...
- 決定: **提出済み＝青**。リストの丸チェック（現状どおり）に合わせ、ヘルプの説明図とカレンダーのドットを変える。
- 対応: 状態→色の表を `src/lib/status.ts` に1本化し、リスト・カレンダー・グラフ・**ヘルプの説明図**が
  全部そこを `import` する。ヘルプに色見本をハードコードしない。
- **残る詰め**: いま「まだ先の未提出」も青（`--primary` 75%）なので、
  カレンダーのドットと棒グラフで **提出済みと未提出（先）が同じ青**になってしまう。
  どちらかをずらす必要がある（濃さで分ける／未提出（先）を無彩色にする／提出済みだけ塗りつぶしにする 等）。
