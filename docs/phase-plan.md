...
Phase 2: サーバーサイド課題保存 + スマート同期                    ⬜ 未着手
...
ログイン済み:
  ...
  2. /api/classroom → DB同期（スマート同期）→ /api/assignments で取得
  3. 結果をIndexedDBにもキャッシュ
未ログイン:
...
### 最適化
- **API呼び出しデデュプ**: 最後の取得から5分以内はキャッシュを返す
- **差分同期**: `updatedAt`比較で変更があった課題のみDB更新
