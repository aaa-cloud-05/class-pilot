...
```
                  ┌─ Web    : classmino.com          → CNAME → Vercel
classmino.com ──────┤
 (Cloudflare で   └─ メール : mail.classmino.com     → TXT(SPF/DKIM) → Resend が送信を許される
  買う＋DNSを持つ)

Google OAuth ── リダイレクトURI https://classmino.com/api/auth/callback/google を許可
...
| **apex（ルートドメイン）** | `classmino.com` そのもの | 仕様上ここに CNAME は置けない（→ §2） |
| **サブドメイン** | `www.classmino.com` `mail.classmino.com` | 用途ごとに名前を分けられる |
...
### つなぐ
1. Vercel の Project → Settings → Domains に `classmino.com` と `www.classmino.com` を追加
2. Vercel が表示する値を Cloudflare の DNS に登録する
   - `www` → **CNAME** → `cname.vercel-dns.com`（Vercel が指示する値をそのまま）
   - apex `classmino.com` → Vercel は A レコードを指示する。Cloudflare なら **CNAME を書いてもよい**
...
### www をどちらかに寄せる
`classmino.com` を正とし、`www` は Vercel の Domains 画面で apex へリダイレクトさせる。
...
### なぜサブドメインを使うのか
メールの「送信ドメイン評判」は**名前ごとに**積み上がる。通知メールを `mail.classmino.com` から出せば、
万一大量に迷惑メール報告されても、`classmino.com`（Webサイトや将来の連絡用メール）の評判を
...
| 項目 | 設定 | 理由 |
|---|---|---|
| **Name** | `mail.classmino.com` | 評判の分離（上記） |
| **Region** | `ap-northeast-1`（東京） | 受信者が日本の大学。**作成後は変更できないのでここで確定させる** |
| **Custom Return-Path** | `send`（既定のまま） | バウンスの受信先。`send.mail.classmino.com` になる。SPF はこの名前に対して評価される |
...
### 手順
1. Resend ダッシュボード → Domains → Add Domain → `mail.classmino.com`
2. 表示された **TXT（SPF）/ TXT（DKIM）/ MX** の値を、Cloudflare の DNS にそのまま登録
   - Cloudflare が「名前」に自動でドメインを補完する場合がある。`mail.classmino.com.classmino.com`
...
```
RESEND_FROM=Classmino <noreply@mail.classmino.com>
...
Cloudflare Email Routing（無料）で `support@classmino.com` を作り、個人の Gmail に転送する:

1. Cloudflare → 対象ドメイン → **Email → Email Routing → Get started**
2. Destination address に個人の Gmail を登録 → 届いた確認メールのリンクを踏む
3. Custom address `support@classmino.com` を作り、転送先に上記を指定
4. Cloudflare が MX と SPF を自動で追加する（Enable ボタン1つ）

結果:
- 受信は無料で個人 Gmail に届く
- 公開されるのは `support@classmino.com` だけ
- `RESEND_REPLY_TO=support@classmino.com` にすれば通知メールへの返信もここに来る

**MX が衝突しないか**: Email Routing は apex（`classmino.com`）に MX を置き、Resend は
`send.mail.classmino.com` に置く。**名前が違うので衝突しない**。SPF も別々の名前に付くため、
「1つの名前に SPF レコードは1つまで」という制約に触れない。

**制約**: Email Routing は**受信専用**で、`support@classmino.com` から送信はできない。
返信すると個人 Gmail のアドレスが相手に見える。避けたい場合は Gmail の
「アカウント → 他のメールアドレスを追加」に Resend の SMTP を登録すれば
`support@classmino.com` として返信できる。問い合わせが実際に来てからでよい。
...
| 項目 | 追加する値 |
|---|---|
| 承認済みの JavaScript 生成元 | `https://classmino.com` |
| 承認済みのリダイレクト URI | `https://classmino.com/api/auth/callback/google` |
...
| 項目 | 値 |
|---|---|
| アプリのホームページ | `https://classmino.com` |
| プライバシーポリシー | `https://classmino.com/privacy` |
| 利用規約 | `https://classmino.com/terms` |
| 承認済みドメイン | `classmino.com` |
...
```
AUTH_URL=https://classmino.com                       # NextAuth のコールバック基準URL
NEXT_PUBLIC_APP_URL=https://classmino.com            # メールの絶対リンク / OGP の metadataBase
RESEND_FROM=Classmino <noreply@mail.classmino.com>
RESEND_REPLY_TO=公開連絡先のアドレス
```
...
nslookup -type=TXT mail.classmino.com
nslookup -type=TXT _dmarc.mail.classmino.com
```
...
4. `https://classmino.com` が Vercel の中身を返し、鍵マークが出る
...
| Resend が Verify されない | レコード名が二重（`mail.classmino.com.classmino.com`）／値の前後に空白／伝播待ち |
