...
最終更新: 2026-09-23 ／ 対象: WebClass（日本データパシフィック）。検証はある1大学の本番環境で実施
...
**認証方式には依存しない。** 検証環境は Shibboleth SSO だが、実装が使うのは
...
## 4.8 実測で確かめたこと（2026-09-23・検証環境の本番）
...
| 他大学のログイン方式 | 検証環境は Shibboleth SSO だが**実装は認証方式に依存しない**（既存のセッションCookieを使うだけ）。他大学が別方式でも動くはず |
