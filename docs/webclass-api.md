...
## 4.8 実測で確かめたこと（2026-09-22・本学の本番環境）

ログイン済みの本人のブラウザから内部 API を直接叩いて確認した。

### 「状態」列の `-` は提出状況の不明ではない

課題実施状況一覧（`/dashboard`）の列は **教材 / 締切 / 実施日 / 最高点 / 状態**。
112行を数えたところ:

| 列 | 値 |
|---|---|
| 状態 | `-` 84 / 回答済み 22 / 未回答 5 / 合格 1 |
| 実施日 | `-` 40 / 日時 72 |

**状態は教員が設定したときだけ入る採点ラベル**で、8割近くが `-`。
提出したかどうかを表すのは **実施日**（＝ `answer_datetime`）で、こちらは必ずどちらかに決まる。

旧実装は「状態」列の文字列を読んでいたため `-` が「不明」になっていた。
**新しい API 方式では不明は発生しない。**
...
| | 件数 |
|---|---|
| `answer_datetime` に値あり（＝提出済み） | 36 |
| `answer_datetime` が `null`（＝未提出） | 26 |
| `undefined` / 空文字 / `scores` が空配列 | **0** |

`hidden_score: true` は14件あったが、**すべて提出済みの行**だった。
点数を隠しているだけで、提出したかどうかとは無関係。
