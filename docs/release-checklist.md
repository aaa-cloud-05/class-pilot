...
- [ ] 🔴 **Supabase のリージョンを東京(ap-northeast-1)へ移す**
      現在 `ap-south-1`（ムンバイ）。**実測で1クエリあたり678ms**。
      ホームを開くだけで数クエリ走るため、体感速度に直結する。
      ユーザーが自分1人・課題243件の今なら、新プロジェクトを作って
      `prisma db push` → 再取り込み で移行できる。人が増えてからでは面倒になる。
