...
import { relativeLabel } from "@/lib/assignment-format"
import { CAT_TEXT } from "@/lib/status"
import { weekHeadline, type WeekState } from "@/lib/week-view"
import { CountUp, SPRING_SOFT } from "@/components/app/motion"
...
export function WeekHero({
  ...
  const weekItems = assignments.filter((a) => a.dueDate && week.days.some((d) => isSameDay(a.dueDate!, d.date)))
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="block truncate text-[13px] text-muted-foreground">
              ...
              {next.dueDate && (
                <>
                  {" ・ "}
                  {isSameDay(next.dueDate, now) ? format(next.dueDate, "HH:mm") : format(next.dueDate, "M/d HH:mm")}
                </>
              )}
            </span>
          </span>
          {nextRel && (
            <span className={cn("shrink-0 text-[13px] font-semibold tabular-nums", CAT_TEXT[nextRel.cat])}>
              {nextRel.text}
            </span>
          )}
