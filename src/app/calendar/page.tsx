...
export default function MockCalendarPage() {
  ...
  const periodItems = useMemo(
    () =>
      assignments.filter(
        (a) => a.dueDate && (mode === "week" ? isSameWeek(a.dueDate, cursor, { weekStartsOn: 1 }) : isSameMonth(a.dueDate, cursor)),
      ),
    [assignments, cursor, mode],
  )
  const periodDone = periodItems.filter((a) => a.submissionState === "submitted").length
  const periodOverdue = periodItems.filter((a) => a.submissionState !== "submitted" && a.dueDate && a.dueDate < now).length
              ...
              {mode === "week" ? (
                desktop ? (
                  <WeekColumns
                    ...
                    onSelect={selectDay}
                    onOpen={(a) => {
                      setSelectedDay(a.dueDate ?? selectedDay)
                      setOpenId(a.id)
                    }}
                    list={assignments}
                    now={now}
