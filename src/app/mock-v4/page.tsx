...
const GROUPS: { title: string; items: { href: string; label: string; desc: string }[] }[] = [
  {
    title: "アプリ",
    items: [
      { href: "/mock-v4/home", label: "ホーム", desc: "今週の進捗・次の1件・締切順のリスト" },
      { href: "/mock-v4/calendar", label: "カレンダー", desc: "週 / 月。PC は課題名まで表示" },
      { href: "/mock-v4/activity", label: "通知", desc: "届いた通知の履歴" },
    ],
  },
  {
    title: "はじめて使うとき",
    items: [
      { href: "/mock-v4/login", label: "ログイン", desc: "Google で続ける / ログインせずに使う" },
      { href: "/mock-v4/start", label: "セットアップ", desc: "Google → WebClass → 通知 の3ステップ" },
      { href: "/mock-v4/import", label: "取り込み", desc: "取り込み中 / 完了 / エラー" },
    ],
  },
  {
    title: "設定",
    items: [
      { href: "/mock-v4/settings", label: "設定（概要）", desc: "接続・通知・コース・アカウント・ヘルプ" },
      { href: "/mock-v4/settings/connections", label: "接続", desc: "Classroom と WebClass。つなぎ方の比較表つき" },
      { href: "/mock-v4/settings/notifications", label: "通知", desc: "届け方とタイミング" },
      { href: "/mock-v4/settings/help", label: "ヘルプ", desc: "ログインの有無・WebClass・通知の比較表" },
    ],
  },
]
...
export default function MockV3Index() {
  return (
    <main className="mx-auto w-full max-w-2xl px-5 pb-16 pt-[max(env(safe-area-inset-top),1.5rem)]">
      <Brand />
      <h1 className="mt-6 text-[20px] font-semibold tracking-[-0.01em]">UI モック v4</h1>
