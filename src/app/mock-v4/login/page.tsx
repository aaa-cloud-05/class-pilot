...
export default function LoginPage() {
  ...
  const enter = (loggedIn: boolean) => {
    ...
    router.push(loggedIn ? "/mock-v4/start" : "/mock-v4/home")
  }
  ...
  return (
    <main className="flex min-h-dvh items-center justify-center px-5 py-10">
      ...
      >
        <Brand size="lg" />
        <h1 className="mt-7 text-[24px] font-semibold leading-snug tracking-[-0.01em]">
          WebClass と Classroom の締切を、ひとつに。
        </h1>
        <ul className="mt-4 space-y-1.5 text-[14px] leading-relaxed text-muted-foreground">
          <li className="flex gap-2">
            <span className="text-foreground">・</span>WebClass の課題にも通知が届く
          </li>
          <li className="flex gap-2">
            <span className="text-foreground">・</span>締切の前にメールとプッシュで知らせる
          </li>
          <li className="flex gap-2">
            <span className="text-foreground">・</span>1週間の忙しさがひと目で分かる
          </li>
        </ul>
