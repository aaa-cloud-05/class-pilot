    ...
    <>
      ...
      <Content className="lg:max-w-xl">
        <div className="space-y-5">
          ...
          <RowGroup>
            <SettingLink
              href="/mock-v4/settings/setup"
              label="セットアップ"
              description="Classroom・WebClass・通知を順につなぐ"
              detail={
            ...
            />
            <SettingLink
              href="/mock-v4/settings/notifications"
              label="通知"
              description="届け方とタイミング"
              detail={notif.enabled ? PRESET[notif.preset] : "オフ"}
            />
            <SettingLink
              href="/mock-v4/settings/courses"
              label="コース"
              description="表示と通知をコースごとに"
              detail={`${courses.filter((c) => !c.hidden).length}件`}
            />
            <SettingLink href="/mock-v4/settings/account" label="アカウント" description="テーマ・ログアウト・削除" />
            <SettingLink href="/mock-v4/settings/help" label="よくある質問" description="困ったときはここから" />
          </RowGroup>
