...
export function buildWeekState(list: MockAssignment[], now: Date, anchor: Date = now): WeekState {
  const start = startOfWeek(anchor, WEEK)
  const inWeek = list.filter((a) => a.due && isSameWeek(a.due, anchor, WEEK))

  const days: DayLoad[] = Array.from({ length: 7 }, (_, i) => {
    const date = addDays(start, i)
    const items = inWeek.filter((a) => a.due && isSameDay(a.due, date))
    return {
      date,
      total: items.length,
      open: items.filter((a) => a.status !== "submitted").length,
  ...
  const done = inWeek.filter((a) => a.status === "submitted").length
  const remaining = total - done
  const overdue = inWeek.filter((a) => a.status !== "submitted" && a.due && a.due < now).length
...
export function nextUp(list: MockAssignment[], now: Date): MockAssignment | null {
  const open = list.filter((a) => a.status !== "submitted" && a.due)
  if (open.length === 0) return null
  const overdue = open.filter((a) => a.due! < now).sort((x, y) => y.due!.getTime() - x.due!.getTime())
  if (overdue.length) return overdue[0]
  return open.filter((a) => a.due! >= now).sort((x, y) => x.due!.getTime() - y.due!.getTime())[0] ?? null
}
