  ...
  return (
    ...
    >
      <Card className="flex gap-3 p-4">
        <ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-muted-foreground" strokeWidth={1.75} aria-hidden />
      ...
      <ArticleSection title="1. コードを用意する">
        <Card className="p-4">
          <div className="flex items-center gap-3">
            <p className="flex-1 text-[15px] text-muted-foreground">登録するコード（どのページで実行しても大丈夫です）</p>
          ...
          <button
            ...
            className="mt-3 flex h-11 w-full items-center gap-2 rounded-control px-1 text-[14px] font-semibold text-primary outline-none hover:bg-muted focus-visible:ring-2 focus-visible:ring-ring"
          >
            <ChevronDown className={cn("h-4 w-4 transition-transform", showCode && "rotate-180")} aria-hidden />
            コードの中身を見る
          </button>
          {showCode && (
            <pre className="mt-2 max-h-48 overflow-auto rounded-control bg-muted p-3 font-mono text-[12px] leading-relaxed text-muted-foreground">
      ...
      <ArticleSection title="3. PC なら自動にもできます" id="auto">
        ...
        <Card className="mt-4 overflow-hidden">
          ...
          <Faq q="それでも動かない">
            Chrome の <code className="rounded bg-muted px-1 font-mono text-[13px]">chrome://extensions</code> で
