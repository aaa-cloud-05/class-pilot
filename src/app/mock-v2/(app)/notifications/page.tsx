...
const KIND: Record<MockNotification["kind"], { icon: LucideIcon; className: string }> = {
  deadline: { icon: AlarmClock, className: "text-muted-foreground" },
  overdue: { icon: AlertTriangle, className: "text-destructive" },
  import: { icon: Download, className: "text-muted-foreground" },
}
...
export default function MockNotificationsPage() {
  ...
  return (
    <>
      <MobileHeader variant="back" title="通知" backHref="/mock-v2/home" actions={readAll} />
      <PageBody desktopTitle="通知" desktopActions={readAll}>
                        ...
                        >
                          ...
                          <span className="min-w-0 flex-1">
                            <span className="flex items-baseline gap-2">
                              ...
                              <span className="shrink-0 text-[13px] tabular-nums text-muted-foreground">{timeAgo(n.at, now)}</span>
                            </span>
        ...
        <p className="mt-6 px-2 text-[13px] leading-relaxed text-muted-foreground">
          通知の受け取り方やタイミングは{" "}
          <Link href="/mock-v2/settings/notifications" className="font-semibold text-primary hover:underline">
            設定 › 通知
          </Link>{" "}
          で変えられます。
        </p>
