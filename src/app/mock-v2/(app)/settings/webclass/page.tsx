...
export default function MockWebclassSettingsPage() {
  ...
  return (
    <>
      <MobileHeader variant="back" title="WebClass 連携" backHref="/mock-v2/settings" />
      <PageBody desktopTitle="WebClass 連携">
        <div className="space-y-7">
          <Card className="p-4">
            <div className="flex items-center gap-3">
              <span className={cn("h-2.5 w-2.5 rounded-full", syncedAt.webclass ? "bg-primary" : "bg-input")} aria-hidden />
          ...
          <section>
            <h2 className="px-4 pb-2 text-[13px] font-semibold text-muted-foreground">自動で取り込む（PC・任意）</h2>
            <Card className="overflow-hidden">
              <p className="px-4 pb-1 pt-4 text-[14px] leading-relaxed text-muted-foreground">
              ...
              {confirmReissue && (
                <div className="mx-4 mb-4 rounded-control border border-border p-3">
                  <p className="text-[14px] font-semibold text-foreground">再発行すると、設定済みの PC では同期が止まります</p>
              ...
              {token && (
                <div className="mx-4 mb-4 rounded-control border border-border p-3">
                  <p className="text-[14px] font-semibold text-foreground">この画面を離れると、もう表示できません。いまコピーしてください</p>
                  <div className="mt-2 flex items-center gap-2">
                    <code className="min-w-0 flex-1 truncate rounded-[8px] bg-card px-3 py-2 font-mono text-[13px]">{token}</code>
              ...
              {!controls.loggedIn && <p className="px-4 pb-4 text-[13px] text-muted-foreground">自動で取り込むには Google でのログインが必要です。</p>}
            </Card>
            <p className="px-4 pt-2 text-[13px] leading-relaxed text-muted-foreground">
              うまくいかないときは{" "}
              <Link href="/mock-v2/help/webclass#auto" className="font-semibold text-primary hover:underline">
                自動取り込みの手順と対処
              </Link>
              を見てください。
            </p>
          ...
          <section>
            <h2 className="px-4 pb-2 text-[13px] font-semibold text-muted-foreground">WebClass の URL</h2>
            <Card className="p-4">
