    ...
    <Sheet
      ...
      footer={
        <div className="space-y-2">
          ...
          <p className="text-center text-[13px] text-muted-foreground">アプリを開くと自動でも更新されます（5分に1回まで）</p>
        </div>
      }
    >
      <div className="space-y-3 pt-2">
        ...
        >
          {controls.data === "reauth" || !controls.loggedIn ? (
            <ButtonLink href="/mock-v2/login" size="sm" onClick={close}>
        ...
        >
          ...
          <ButtonLink href="/mock-v2/help/webclass" variant="ghost" size="sm" onClick={close}>
            取り込み方法
          </ButtonLink>
        </SourceBlock>
        ...
        <p className="px-1 pt-1 text-[13px] leading-relaxed text-muted-foreground">
          ...
          <Link href="/mock-v2/help/sync" onClick={close} className="font-medium text-primary hover:underline">
            同期のしくみ
          </Link>
        </p>
      </div>
  ...
  return [
    {
      ...
      href: "/mock-v2/login",
      done: all || controls.loggedIn,
    },
    {
      key: "webclass",
      title: "WebClass をつなぐ",
      desc: "ブックマークに1つ登録するだけ",
      href: "/mock-v2/help/webclass",
    ...
    },
    {
      ...
      href: "/mock-v2/settings/notifications",
...
const TABS = [
  { href: "/mock-v2/home", label: "ホーム", icon: House, match: ["/mock-v2/home", "/mock-v2/notifications"] },
  { href: "/mock-v2/calendar", label: "カレンダー", icon: CalendarDays, match: ["/mock-v2/calendar"] },
  { href: "/mock-v2/settings", label: "設定", icon: Settings, match: ["/mock-v2/settings", "/mock-v2/help"] },
]
...
function Sidebar() {
  ...
  const items = [
    { href: "/mock-v2/home", label: "ホーム", icon: House, match: ["/mock-v2/home"] },
    { href: "/mock-v2/calendar", label: "カレンダー", icon: CalendarDays, match: ["/mock-v2/calendar"] },
    { href: "/mock-v2/notifications", label: "通知", icon: Bell, match: ["/mock-v2/notifications"], badge: unread },
    { href: "/mock-v2/settings", label: "設定", icon: Settings, match: ["/mock-v2/settings", "/mock-v2/help"] },
  ]
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-[240px] flex-col border-r border-border bg-card px-3 pb-4 pt-5 lg:flex">
      <Link href="/mock-v2/home" className="rounded-control px-2 outline-none focus-visible:ring-3 focus-visible:ring-ring/40">
        <Brand />
      </Link>
      ...
      <div className="mt-auto space-y-2">
        <div className="rounded-card border border-border p-3">
          ...
          <Button variant="ghost" size="sm" className="mt-2 w-full" onClick={refresh} disabled={!controls.loggedIn || syncing}>
            <RefreshCw className={cn("h-3.5 w-3.5", syncing && "animate-spin")} aria-hidden />
            {syncing ? "更新中…" : "今すぐ更新"}
          </Button>
        </div>
        <Link
          href="/mock-v2/settings"
        ...
        >
          ...
          <span className="min-w-0">
            <span className="block truncate text-[13px] font-medium">{controls.loggedIn ? "佐藤 ひなた" : "ログインしていません"}</span>
  ...
  return (
    <header className="sticky top-0 z-20 bg-background/85 backdrop-blur-xl lg:hidden">
      <div className="mx-auto flex min-h-[52px] max-w-xl items-center gap-1 px-2 pt-[env(safe-area-inset-top)]">
        {variant === "home" && (
          <>
            <Link href="/mock-v2/home" className="ml-2 flex-1 rounded-control outline-none focus-visible:ring-3 focus-visible:ring-ring/40">
              <Brand />
            </Link>
            <SyncButton />
            <Link
              href="/mock-v2/notifications"
    ...
    return (
      <div className="mb-4">
        <div className="flex items-center gap-3 rounded-card bg-card px-4 py-3 shadow-card" role="alert">
          ...
          <ButtonLink href="/mock-v2/login" size="sm">
            再ログイン
          </ButtonLink>
        </div>
      </div>
