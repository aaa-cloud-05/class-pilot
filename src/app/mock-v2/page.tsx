...
const SCREENS: { group: string; items: { href: string; title: string; desc: string }[] }[] = [
  {
    group: "タブ",
    items: [
      { href: "/mock-v2/home", title: "ホーム", desc: "件数サマリ・はじめの設定・締切でまとめた課題リスト・詳細シート" },
      { href: "/mock-v2/calendar", title: "カレンダー", desc: "週 / 月の切替・選んだ日の課題・提出の進み具合" },
      { href: "/mock-v2/settings", title: "設定", desc: "アカウント・連携・通知・コース・テーマ・ヘルプ・削除" },
    ],
  },
  {
    group: "ホームから開く画面",
    items: [
      { href: "/mock-v2/notifications", title: "通知", desc: "ベルから開く通知の履歴（PC はサイドバー）" },
    ],
  },
  {
    group: "設定の中",
    items: [
      { href: "/mock-v2/settings/notifications", title: "通知の設定", desc: "全体オン/オフ・プッシュ・メール・タイミング・ミュート中の課題" },
      { href: "/mock-v2/settings/courses", title: "コース", desc: "コースごとの通知と表示" },
      { href: "/mock-v2/settings/webclass", title: "WebClass 連携", desc: "取り込み状況・自動取り込み（PC）・URL" },
      { href: "/mock-v2/help", title: "はじめかたと使い方", desc: "はじめの設定チェックリストとガイド一覧" },
      { href: "/mock-v2/help/webclass", title: "ガイド：WebClass の取り込み方", desc: "読みやすさを見直した記事レイアウト" },
    ],
  },
  {
    group: "ナビの外",
    items: [
      { href: "/mock-v2/login", title: "ログイン", desc: "PC は左右2分割" },
      { href: "/mock-v2/import", title: "取り込み画面", desc: "取り込み中・完了・エラー" },
    ],
  },
]
...
export default function MockIndexPage() {
  return (
    <main className="mx-auto w-full max-w-2xl px-4 pb-24 pt-[max(env(safe-area-inset-top),1.5rem)]">
      <Brand />
      <h1 className="mt-8 text-[26px] font-semibold tracking-[-0.02em]">UI モック v2</h1>
