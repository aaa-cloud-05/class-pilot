    ...
    <main className="flex min-h-dvh flex-col px-6 pb-[max(env(safe-area-inset-bottom),1.5rem)] pt-[max(env(safe-area-inset-top),1.5rem)]">
      <div className="mx-auto flex w-full max-w-sm flex-1 flex-col">
        ...
        <div className="flex flex-1 flex-col justify-center">
          {phase === "importing" && (
            <div aria-live="polite">
              ...
              <p className="mt-2 text-[15px] text-ink-2">このままお待ちください。数秒で終わります。</p>
              <div className="mt-8 h-2 overflow-hidden rounded-full bg-surface-2">
                  ...
                  <li key={s} className="flex items-center gap-3 text-[15px]">
                    <span
                      className={cn(
                        ...
                        i < stage ? "bg-ok text-white" : i === stage ? "border-2 border-brand" : "border-2 border-line-strong",
                      )}
                      aria-hidden
                    >
                      {i < stage && <Check className="h-3.5 w-3.5" strokeWidth={3} />}
                    </span>
                    <span className={cn(i <= stage ? "font-semibold text-ink" : "text-ink-3")}>{s}</span>
                  </li>
                ))}
              </ol>
            </div>
          ...
          {phase === "done" && (
            <div className="text-center animate-in fade-in-0 zoom-in-95 duration-300">
              ...
              <span className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-ok text-white">
                <Check className="h-10 w-10" strokeWidth={3} aria-hidden />
              </span>
              <p className="mt-6 text-[24px] font-bold">12 件を取り込みました</p>
              <p className="mt-2 text-[15px] text-ink-2">新しい課題が 2 件、締切が変わった課題が 1 件あります。</p>
              <ButtonLink href="/mock/home" size="lg" className="mt-8 w-full">
                ホームで確認する
              </ButtonLink>
            </div>
          )}
          ...
          {phase === "error" && (
            <div className="animate-in fade-in-0 duration-300">
              ...
              <p className="mt-3 rounded-control bg-surface-2 px-3 py-2 font-mono text-[12px] text-ink-3">エラー 401: session expired</p>
              <div className="mt-8 space-y-3">
                <Button size="lg" className="w-full" onClick={() => setPhase("importing")}>
                  もう一度試す
                </Button>
                <ButtonLink href="/mock/help/webclass" variant="ghost" size="lg" className="w-full">
                  取り込み方を確認する
                </ButtonLink>
              </div>
            </div>
        ...
        <div className="rounded-card border border-dashed border-line-strong p-3">
          <p className="mb-2 text-center text-[12px] text-ink-3">モック用：表示を切り替え</p>
          <Segmented<Phase>
            label="取り込み画面の状態"
            size="sm"
            value={phase}
