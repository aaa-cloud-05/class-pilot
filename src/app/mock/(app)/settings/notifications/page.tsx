...
export default function MockNotificationSettingsPage() {
  ...
  return (
    <>
      <MobileHeader variant="back" title="通知" backHref="/mock/settings" />
        ...
        <div className="space-y-7">
            ...
            {PRESETS.map((p) => {
              ...
              return (
                <RowButton
                  ...
                  detail={
                    <Check
                      className={cn("ml-auto h-5 w-5 text-brand-text", on ? "opacity-100" : "opacity-0")}
                      strokeWidth={2.6}
          ...
          <ListGroup
            ...
            footer={
              <>
                コースごとにまとめて止めるときは{" "}
                <Link href="/mock/settings/courses" className="font-semibold text-brand-text hover:underline">
                  コース
                </Link>{" "}
                から。
              </>
          ...
          >
            {muted.length === 0 ? (
              <RowStatic label={<span className="text-ink-3">ミュート中の課題はありません</span>} />
            ) : (
              muted.map((a) => (
                <RowStatic
                  key={a.id}
