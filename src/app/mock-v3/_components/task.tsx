...
export function StatusCircle({ a, className }: { a: MockAssignment; className?: string }) {
  ...
  return (
    <button
      ...
      className={cn(
        "group relative flex h-9 w-9 shrink-0 items-center justify-center rounded-md outline-none focus-visible:ring-2 focus-visible:ring-ring/60",
        className,
      )}
    >
      <motion.span
...
export function TaskRow({
  ...
  return (
    <div className={cn("flex items-center gap-2 bg-background pl-2 pr-3 transition-colors", selected ? "bg-accent" : "hover:bg-accent/60")}>
      <StatusCircle a={a} />
      <button
        type="button"
        onClick={() => onOpen(a)}
        className="flex min-h-[52px] min-w-0 flex-1 items-center gap-3 py-2 text-left outline-none focus-visible:ring-2 focus-visible:ring-ring/60 lg:min-h-[42px]"
      >
        <span className="min-w-0 flex-1">
          <span className="flex items-center gap-1.5">
            <span className={cn("truncate text-[15px] lg:text-[14px]", state === "done" ? "text-muted-foreground" : "text-foreground")}>
              {a.title}
            </span>
            {a.muted && <BellOff className="h-3.5 w-3.5 shrink-0 text-muted-foreground/70" aria-label="通知オフ" />}
          </span>
          <span className="mt-0.5 block truncate text-[13px] text-muted-foreground lg:hidden">{course?.name}</span>
        </span>
        <span className="hidden w-32 shrink-0 truncate text-[13px] text-muted-foreground xl:block">{course?.name}</span>
        <span className={cn("num w-[4.75rem] shrink-0 text-right text-[13px]", due.className)}>{due.text}</span>
      </button>
    </div>
  )
}
  ...
  return (
    <div className="bg-background">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        className="flex h-10 w-full items-center gap-1.5 px-4 text-[13px] text-muted-foreground outline-none transition-colors hover:bg-accent focus-visible:bg-accent"
      >
...
export function TaskGroups({
  ...
  return (
    <Panel className="divide-y divide-border overflow-hidden">
      {groups.map((g) => (
        <section key={g.key}>
          <h3 className="flex items-center gap-2 px-4 py-1.5 text-[12px] font-medium text-muted-foreground">
            <span className={cn(g.key === "overdue" && "text-destructive")}>{GROUP_LABEL[g.key]}</span>
            <span className="num">{g.items.length}</span>
          </h3>
          <ul className="divide-y divide-border border-y border-border">
            <AnimatePresence initial={false}>
              {g.items.map((a) => (
                <motion.li key={a.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, height: 0 }}>
