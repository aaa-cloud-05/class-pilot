...
import { Button, ButtonLink, Card, IconButton, INPUT, Sheet } from "./ui"
...
export function SyncSheet() {
  const { syncOpen, setSyncOpen, syncedAt, now, controls, webclassUrl, setWebclassUrl, showToast } = useMock()
      ...
      <div className="space-y-3 pt-2">
        ...
        >
          {controls.data === "reauth" || !controls.loggedIn ? (
            <ButtonLink href="/mock-v5/login" size="sm" onClick={close}>
...
function Sidebar() {
  const pathname = usePathname()
  const { setAddOpen, notifications, syncedAt, now, controls, setSyncOpen } = useMock()
  const { classroom, webclass } = useSyncSummary()
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-[240px] flex-col border-r border-border bg-card px-3 pb-4 pt-5 lg:flex">
      <Link href="/mock-v5/home" className="rounded-control px-2 outline-none focus-visible:ring-3 focus-visible:ring-ring/40">
        <Brand />
      </Link>
  ...
  return (
    <header className="sticky top-0 z-20 bg-background/85 backdrop-blur-xl lg:hidden">
      <div className="mx-auto flex min-h-[52px] max-w-xl items-center gap-1 px-2 pt-[env(safe-area-inset-top)]">
        {variant === "home" && (
          <>
            <Link href="/mock-v5/home" className="ml-2 flex-1 rounded-control outline-none focus-visible:ring-3 focus-visible:ring-ring/40">
              <Brand />
            </Link>
            <SyncButton />
            <Link
              href="/mock-v5/notifications"
    ...
    return (
      <div className="mb-4">
        <div className="flex items-center gap-3 rounded-card bg-card px-4 py-3 shadow-card" role="alert">
          ...
          <ButtonLink href="/mock-v5/login" size="sm">
            再ログイン
          </ButtonLink>
        </div>
      </div>
