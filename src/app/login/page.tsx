  ...
  return (
    <main className="min-h-dvh lg:grid lg:grid-cols-2">
      <section className="hidden flex-col justify-between bg-card p-12 lg:flex">
        <Link href="/" aria-label="ホームへ" className="w-fit rounded-control outline-none focus-visible:ring-3 focus-visible:ring-ring/40">
          <Brand size="lg" />
        </Link>
        <div>
          ...
          <ul className="mt-10 space-y-6">
            {POINTS.map(({ icon: Icon, title, desc }) => (
              <li key={title} className="flex gap-4">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-control bg-muted text-muted-foreground">
                  <Icon className="h-[18px] w-[18px]" strokeWidth={1.75} aria-hidden />
                </span>
                <span>
                  <span className="block text-[17px] font-bold">{title}</span>
                  <span className="mt-0.5 block text-[15px] text-muted-foreground">{desc}</span>
                </span>
              </li>
            ))}
          </ul>
        </div>
        <p className="text-[13px] text-muted-foreground">UnionFetch は Google・WebClass とは関係のない非公式ツールです。</p>
      </section>
          ...
          <div className="flex flex-1 flex-col justify-center py-10 lg:flex-none lg:py-0">
            ...
            <div className="mt-8 space-y-3">
              <button
                ...
                className="flex h-[52px] w-full items-center justify-center gap-3 rounded-control bg-card text-[16px] font-semibold text-foreground shadow-card outline-none transition-colors hover:bg-muted focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.98]"
              >
                <GoogleMark />
                Google でログイン
              </button>
